...
const ProdutoForm = () => {
    ...
    return (
        <Box component="form" onSubmit={handleSubmit(onSubmit)} sx={{ backgroundColor: "#ed8f24", padding: 2, borderRadius: 1, mt: 2 }}>
            <Toolbar sx={{ backgroundColor: "#ed8f24", padding: 1, borderRadius: 2, mb: 2, display: 'flex', justifyContent: 'space-between' }}>
                <Typography variant="h6" gutterBottom color="#804b0f">{title}</Typography>
            </Toolbar>
            <Box sx={{ backgroundColor: 'white', padding: 2, borderRadius: 3, mb: 2 }}>
                ...
                <Box sx={{ mt: 2 }}>
                    ...
                    <Button variant="contained" component="label" disabled={isReadOnly}>
                        Selecione a Imagem
                        <input type="file" accept="image/*" onChange={handleFileChange} disabled={isReadOnly} style={{ marginTop: '8px' }} hidden={true} />
                    </Button>
                </Box>
